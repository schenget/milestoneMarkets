import {
  ...
  StrategyName,
  StrategyOutput
} from "@milestone/core-types";

const clamp = (value: number, min = 0, max = 100) => Math.max(min, Math.min(max, value));

const chooseAction = (score: number): "BUY" | "SELL" | "HOLD" => {
...
export class StrategyEngine {
  run(input: SignalInput, config: StrategyConfig): StrategyOutput {
    const byStrategy: Record<StrategyName, (i: SignalInput, c: StrategyConfig) => StrategyOutput> = {
  ...
  private advancedQuantitative = (input: SignalInput, config: StrategyConfig): StrategyOutput => {
    ...
    return {
      ...
      explanation:
        "Advanced Quantitative Strategy combines machine learning probability, sentiment and volatility-adjusted momentum to identify short-term opportunities.",
      symbol: input.symbol,
  ...
  private technicalFundamentalHybrid = (input: SignalInput, config: StrategyConfig): StrategyOutput => {
    const f = input.marketFeatures;
    const rsi = f.rsi ?? 50;
    const macd = f.macd ?? 0;
    const pe = f.pe ?? 15;
    ...
    return {
      ...
      risk: riskFromConfidence(confidence),
      explanation:
        "Technical-Fundamental Hybrid Strategy blends RSI, MACD, liquidity and valuation quality filters to balance timing and business quality.",
      symbol: input.symbol,
      country: input.country,
      generatedAt: new Date().toISOString()
    };
  ...
  private longHorizonValue = (input: SignalInput, config: StrategyConfig): StrategyOutput => {
    const f = input.marketFeatures;
    ...
    return {
      ...
      risk: riskFromConfidence(confidence),
      explanation:
        "Long-Horizon Value Strategy emphasizes dividend reliability, lower volatility and macro-consistent earnings resilience for durable compounding.",
      symbol: input.symbol,
      country: input.country,
      generatedAt: new Date().toISOString()
    };
